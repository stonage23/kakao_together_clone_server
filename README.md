...
- [🏷️ 게시글](#게시글)
  - 임시저장 기능으로 사용자 경험 향상
- [🏷️ 파일업로드](#파일업로드)
  - AWS S3, CDN(CloudFront) 연동하여 정적 리소스 접근 보안 확립 및 캐싱
  - 임시파일, 사용파일 스토리지 분리
  - 저장소 추가 시 확장성을 고려한 설계
...
파일 저장소 로직을 추상화 계층으로 설계하여, 로컬 저장소부터 AWS S3 까지 다양한 저장소를 plug-in 방식으로 유연하게 지원하도록 설계하였습니다. 그 결과 향후 추가될 저장소에 대한 확장성을 확보하였습니다. <br>
[파일저장소서비스](src/main/java/com/kakao/together/file/storage)<br>
[파일접근URL resolver](src/main/java/com/kakao/together/file/resolver)
