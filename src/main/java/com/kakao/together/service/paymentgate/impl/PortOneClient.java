...
import com.kakao.together.controller.paymentgate.dto.CustomCancelData;
import com.kakao.together.controller.paymentgate.dto.PaymentResponse;
import com.kakao.together.exception.paymentgate.PaymentGateResponseException;
import com.kakao.together.exception.paymentgate.PaymentGateTokenException;
import com.kakao.together.exception.paymentgate.PaymentNotFoundException;
...
@Component
class PortOneClient implements PaymentGateClient {
    ...
    public PortOneClient(final PortOneTokenProvider portOneTokenProvider, @Value("${imp.api.key}") final String key, @Value("${imp.api.secret}") final String secret, final PaymentInternalHandler paymentInternalHandler) {
    ...
    public PaymentResponse getPayment(String impUid) {
        ...
        if (paymentResponse == null || paymentResponse.getMerchantUid() == null) {
            throw new PaymentNotFoundException("PG 데이터 조회 실패 또는 merchantUid가 존재하지 않음; impUid: {}" + impUid);
        }
    ...
    public PaymentResponse refundPayment(String impUid) {
        ...
        if (response.getResponse() == null) {
            throw new PaymentNotFoundException(response.getMessage());
        }
